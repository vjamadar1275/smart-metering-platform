...
As of Phase 4, `terraform/` defines real infrastructure (networking, Key Vault, managed identities, ADLS Gen2, the Databricks workspace, Unity Catalog, and Event Hub) for dev/staging/prod — but nothing has been applied to live Azure yet from this repo. SQL Warehouses and full Monitoring remain interface-only until Phases 6/8. Bronze and Silver are real Lakeflow Declarative Pipelines (`src/pipelines/`, deployed via `databricks.yml` + `bundles/`); `reference.dim_meter`/`dim_customer`/`dim_dma` (`sql/ddl/reference/`) are directly-runnable seed scripts a human runs once per environment after Unity Catalog exists, since they have no upstream pipeline feed.
