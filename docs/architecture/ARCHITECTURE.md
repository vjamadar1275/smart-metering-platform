...
| [0012](../decisions/ADR-0012-phase7-ml-model-design-and-nl-agent-safety.md) | Unsupervised/narrowly-scoped ML models on synthetic data; `sqlglot`-based structural SQL safety for the NL agent | Inventing synthetic failure/leak labels for "proper" supervised models; regex/keyword-based SQL safety checks | Honest model claims beat fabricated ground truth; real SQL parsing closes alias-vs-schema ambiguity a regex demonstrably got wrong |
